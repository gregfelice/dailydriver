...
Reverts:   snapshots pre-apply state to ~/.config/nightpanel/nightpanel-state.json and restores it
...
class NightpanelOrchestrator:
    ...
    def install_gnome_extension(self) -> bool:
        ...
        ext_src  = Path(__file__).parent.parent.parent.parent.parent / \
                   ".local" / "share" / "gnome-shell" / "extensions" / "nightpanel@nightpanel"
    ...
    def update_brightness(self, brightness: float) -> None:
        ...
        active_file = Path.home() / ".config" / "nightpanel" / "nightpanel-active"
